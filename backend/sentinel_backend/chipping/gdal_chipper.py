...
def chip_affine(source_transform, col_off, row_off) -> rasterio.transform.Affine:
    ...
    The chip origin is computed via proper affine matrix multiplication so
    that all six transform coefficients — including the rotation terms ``.b``
    and ``.d`` — are handled correctly for non-north-up imagery:

        chip_transform = source_transform * Affine.translation(col_off, row_off)
    ...
    """
    return source_transform * Affine.translation(col_off, row_off)
