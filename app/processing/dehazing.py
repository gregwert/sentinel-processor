...
def detect_clouds_simple(img_uint8: np.ndarray, brightness_thresh: float = 0.75, saturation_thresh: float = 0.08) -> np.ndarray:
    ...
    brightness_thresh : float, optional
        Mean channel value (normalised 0-1) above which a pixel is considered
        a cloud candidate; default 0.75.
    saturation_thresh : float, optional
        Max-minus-min channel spread (normalised 0-1) below which a pixel is
        considered spectrally neutral (white/grey), reinforcing the cloud
        classification; default 0.08.

    Returns
    -------
    np.ndarray
        Shape (H, W), dtype bool. True where a pixel is classified as cloud.
    """
...
def dark_channel(img_f32: np.ndarray, patch_size: int = 15) -> np.ndarray:
    ...
    ----------
    img_f32 : np.ndarray
        Shape (H, W, 3), dtype float32, values in [0, 1]. Input image
        (or normalised radiance estimate).
    patch_size : int, optional
        Side length of the rectangular erosion kernel used as the local
        neighbourhood window. Must be a positive odd integer; default 15.

    Returns
    -------
    np.ndarray
        Shape (H, W), dtype float32. Per-pixel minimum over all channels and
...
def estimate_atmospheric_light(img_f32: np.ndarray, dark: np.ndarray, top_percent: float = 0.001, cloud_mask: np.ndarray = None) -> np.ndarray:
    ...
    img_f32 : np.ndarray
        Shape (H, W, 3), dtype float32, values in [0, 1]. Input image.
    dark : np.ndarray
        Shape (H, W), dtype float32. Pre-computed dark channel of ``img_f32``.
    top_percent : float, optional
        Fraction of non-cloud pixels with the highest dark-channel values used
        as atmospheric light candidates; default 0.001.
    cloud_mask : np.ndarray or None, optional
        Shape (H, W), dtype bool. If provided, cloud pixels are excluded from
        candidate selection. Falls back to all pixels when more than 80 % of the
        scene is flagged as cloud; default None.

    Returns
    -------
    np.ndarray
        Shape (3,), dtype float32. RGB atmospheric light estimate A, taken as
...
def transmission_map(img_f32: np.ndarray, A: np.ndarray, patch_size: int = 15, omega: float = 0.95) -> np.ndarray:
    ...
    ----------
    img_f32 : np.ndarray
        Shape (H, W, 3), dtype float32, values in [0, 1]. Input hazy image.
    A : np.ndarray
        Shape (3,), dtype float32. Atmospheric light estimate.
    patch_size : int, optional
        Patch size forwarded to :func:`dark_channel`; default 15.
    omega : float, optional
        Haze retention factor in [0, 1]. Values close to 1 remove more haze
        but may over-darken already dark regions; default 0.95.

    Returns
    -------
    np.ndarray
        Shape (H, W), dtype float32. Raw (unrefined) transmission map with
        values in (0, 1].
    """
...
def guided_filter_transmission(guide: np.ndarray, transmission: np.ndarray, radius: int = 60, eps: float = 1e-3) -> np.ndarray:
    ...
    ----------
    guide : np.ndarray
        Shape (H, W), dtype float32, values in [0, 1]. Greyscale guidance image
        (typically the luminance channel of the hazy input).
    transmission : np.ndarray
        Shape (H, W), dtype float32, values in [0, 1]. Raw transmission map to
        be refined.
    radius : int, optional
        Guided-filter window radius in pixels. Larger values produce smoother
        results at the cost of stronger halo suppression; default 60.
    eps : float, optional
        Regularisation parameter controlling the degree of edge preservation.
        Smaller values preserve more edges; default 1e-3.

    Returns
    -------
    np.ndarray
        Shape (H, W), dtype float32. Edge-preserving refined transmission map.
...
def recover_scene_radiance(img_f32: np.ndarray, t: np.ndarray, A: np.ndarray, t0: float = 0.1) -> np.ndarray:
    ...
    ----------
    img_f32 : np.ndarray
        Shape (H, W, 3), dtype float32, values in [0, 1]. Hazy input image.
    t : np.ndarray
        Shape (H, W), dtype float32. Refined transmission map.
    A : np.ndarray
        Shape (3,), dtype float32. Atmospheric light estimate.
    t0 : float, optional
        Lower bound on transmission to avoid division by near-zero values and
        excessive noise amplification in very dense haze; default 0.1.

    Returns
    -------
    np.ndarray
        Shape (H, W, 3), dtype float32, values clipped to [0, 1]. Recovered
...
class Dehazer:
    ...
    ----------
    patch_size : int
        Side length of the local minimum-filter patch used for dark channel
        computation and transmission estimation. Must be a positive odd integer;
        default 15.
    omega : float
        Haze retention factor. Values close to 1 remove more haze but risk
        over-darkening already dark pixels; default 0.95.
    t0 : float
        Minimum transmission clamp applied before scene radiance recovery to
        prevent division-by-zero and noise amplification; default 0.1.
    use_guided_filter : bool
        When True the raw transmission map is refined by a guided (or bilateral)
        filter to suppress halo artefacts at depth discontinuities; default True.
    mask_clouds : bool
        When True cloud pixels are detected before dehazing and restored from
        the original image afterwards to prevent artefacts on bright surfaces;
        default True.
    brightness_thresh : float
        Mean normalised brightness threshold forwarded to
        :func:`detect_clouds_simple`; default 0.75.
    saturation_thresh : float
        Max-minus-min normalised saturation threshold forwarded to
    ...
    def run(self, img: np.ndarray) -> np.ndarray:
        ...
        ----------
        img : np.ndarray
            Shape (H, W, 3), dtype uint8. Hazy input RGB image.

        Returns
        -------
        np.ndarray
            Shape (H, W, 3), dtype uint8. Dehazed image with cloud pixels
...
def dehaze(
    ...
    img_uint8 : np.ndarray
        Shape (H, W, 3), dtype uint8. Hazy input RGB image.
    patch_size : int, optional
        Forwarded to :class:`Dehazer`; default 15.
    omega : float, optional
        Forwarded to :class:`Dehazer`; default 0.95.
    t0 : float, optional
        Forwarded to :class:`Dehazer`; default 0.1.
    use_guided_filter : bool, optional
        Forwarded to :class:`Dehazer`; default True.
    mask_clouds : bool, optional
        Forwarded to :class:`Dehazer`; default True.
    brightness_thresh : float, optional
        Forwarded to :class:`Dehazer`; default 0.75.
    saturation_thresh : float, optional
        Forwarded to :class:`Dehazer`; default 0.08.

    Returns
    -------
    np.ndarray
        Shape (H, W, 3), dtype uint8. Dehazed image returned by
        :meth:`Dehazer.run`.
    """
    return Dehazer(
