...
def read_sentinel_tiff(path: str, band_indices=(1, 2, 3)) -> Tuple[np.ndarray, dict]:
    ...
    ----------
    path : str
        Filesystem path to a GeoTIFF file readable by rasterio.
    band_indices : tuple of int, optional
        1-based rasterio band indices to read, e.g. ``(1, 2, 3)`` for RGB.
        All indices must be <= the number of bands in the file; default (1, 2, 3).

    Returns
    -------
    arr : np.ndarray
        Shape (H, W, C), dtype uint16. Pixel values in sensor-native DN units,
        one slice per requested band in the order given by ``band_indices``.
    meta : dict
        rasterio metadata dict with CRS, transform, and band count updated to
        match the selected bands.

    Raises
    ------
    ValueError
        If any requested band index exceeds the number of bands present in the
        file.
    """
...
def percentile_stretch(arr: np.ndarray, p_low: float = 2.0, p_high: float = 98.0, per_band: bool = True) -> np.ndarray:
    ...
    ----------
    arr : np.ndarray
        Shape (H, W, C), dtype uint16. Input imagery in sensor DN units.
    p_low : float, optional
        Lower percentile used as the clip minimum. Pixels at or below this
        percentile are mapped to 0; default 2.0.
    p_high : float, optional
        Upper percentile used as the clip maximum. Pixels at or above this
        percentile are mapped to 255; default 98.0.
    per_band : bool, optional
        If True, percentiles are computed independently for each band so that
        each channel fills the full 0-255 range. If False, a single pair of
        percentiles is computed across all bands jointly; default True.

    Returns
    -------
    np.ndarray
        Shape (H, W, C), dtype uint8. Contrast-stretched image ready for
