...
def chip_lat_lon_bounds(
    ...
    # Compute the chip's affine transform (origin at chip top-left corner)
    origin_x = transform.c + col_off * transform.a
    origin_y = transform.f + row_off * transform.e  # transform.e is negative for north-up

    # Four corners in source CRS
    corners_src = [
        (origin_x, origin_y),
        (origin_x + chip_w * transform.a, origin_y),
        (origin_x, origin_y + chip_h * transform.e),
        (origin_x + chip_w * transform.a, origin_y + chip_h * transform.e),
    ]
